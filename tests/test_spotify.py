...
class TestSpotify(unittest.TestCase):
    ...
    def test_c_artists_info(self):
        """ Tests spotify.get_artists_info method """

        artists_id = ['4gzpq5DPGxSnKTe4SA8HAU', '53A0W3U0s8diEn9RhXQhVz']
        artists_info = self.spotify.get_artists_info(artists_id)
        self.assertTrue(len(artists_info)==2)
        self.assertTrue('artist_followers_total' in artists_info[0])

    def test_d_artist_top_tracks_info(self):
        """ Tests spotify.get_artist_top_tracks_info method """

        artist_id = '4gzpq5DPGxSnKTe4SA8HAU'
        market = 'US'
        tracks_info = self.spotify.get_artist_top_tracks_info(artist_id, market)
